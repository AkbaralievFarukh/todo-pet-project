const addDoBtn = document.querySelector('#add-do__btn')
const addDoInput = document.querySelector('#add-do__input')
const tasksList = document.querySelector('.tasks-list')
const taskEmpty = document.querySelector('.tasks-empty')
...
let allDo = () => {
    ...
    ${doItem.map((doItem, index) => {
        ...
        <div class="tasks-list__item">
            <div>
                <input type="checkbox" class="tasks-list__item-checkbox" id="tasks-checkbox${countCheckbox}" ${doItem.completed ? 'checked' : ''} onchange="toggleTaskCompleted(${index})">
                <label for="tasks-checkbox${countCheckbox}"></label>
                <p class="tasks-list__item-text">${doItem.text}</p>
            </div>
            <button class="tasks-list__item-delete" onclick="deleteTask(${index})">
    ...
    `
    updateCounters(); // Добавляем обновление счетчиков после отрисовки задач

}

const updateCounters = () => {
    const totalTasksCounter = document.querySelector('.tasks-info__counter');
    const completedTasksCounter = document.querySelector('.tasks-info__completed-counter');

    if (totalTasksCounter && completedTasksCounter) {
        let completedTasks = doItem.filter(task => task.completed).length;

        totalTasksCounter.textContent = doItem.length.toString();
        completedTasksCounter.textContent = completedTasks.toString();
    } else {
        console.error("Cannot find elements for updating counters.");
    }
...
const showCompletedTasks = () => {
    ...
    const completedTasks = doItem.filter(task => task.completed);
    tasksList.innerHTML = `
        ${completedTasks.map((doItem, index) => {
        const taskItem = document.createElement('div');
        taskItem.classList.add('tasks-list__item');
        return `
            <div class="tasks-list__item">
                <div>
                    <input type="checkbox" class="tasks-list__item-checkbox" id="tasks-checkbox${index}" checked disabled>
                    <label for="tasks-checkbox${index}"></label>
                    <p class="tasks-list__item-text">${doItem.text}</p>
                </div>
                <button class="tasks-list__item-delete" onclick="deleteTask(${index})">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    `;
    updateCounters(); // Update counters for completed tasks
};



const addDo = () => {
    let id = doItem.length + 1;
    let newDo = {
        id: id++,
        text: addDoInput.value,
        completed: false
    };
    doItem.unshift(newDo);
    updateCounters(); // Обновляем счетчики сразу после добавления новой задачи
    allDo();
    addDoInput.value = ""; // Очищаем поле ввода после добавления задачи
    if (doItem.length < 1) {
...
const toggleTaskCompleted = (index) => {
    doItem[index].completed = !doItem[index].completed;
    updateCounters(); // Обновляем счетчики сразу после изменения состояния задачи
    allDo();
};
...
allDo()
updateCounters(); // Add this line to initialize counters
